...
class ReactiveAgent(object):
    ...
    def __init__(self, _RAL_x, _RAL_y, _local_x, _local_y, _img_array):
    ...
    def Is_Inside_Image_Frame(self):
        ...
        if (
            self.global_x < 0
            or self.global_x >= self.img_array.shape[1]
            or self.global_y < 0
            or self.global_y >= self.img_array.shape[0]
        ):
...
class ReactiveAgent_Leader(object):
    ...
    def __init__(
        self, _x, _y, _img_array, _group_size=50, _group_step=5, _field_offset=[0, 0]
    ):
        ...
        self.field_recorded_positions = [
            [self.x + int(self.field_offset[0]), self.y + int(self.field_offset[1])]
        ]
    ...
    def RAs_square_init(self):
        ...
        self.RA_list = []
        for i in range(
            -self.group_size, self.group_size + self.group_step, self.group_step
        ):
    ...
    def Get_RAs_Mean_Point(self):
        ...
        self.recorded_Decision_Score += [
            active_RA_counter / (self.nb_RAs - nb_outside_frame_RAs)
        ]
    ...
    def Move_Based_on_AD_Order(self, _ADO_x, _ADO_y):
        ...
        self.field_recorded_positions += [
            [int(self.x + self.field_offset[0]), int(self.y + self.field_offset[1])]
        ]
    ...
    def Compute_Surface(self):
        ...
        while nb_explorers > 0:
            print_row = (
                explorers[0][1] + self.group_size
            )  # row coord in surface print array
            print_col = (
                explorers[0][0] + self.group_size
            )  # column coord in surface print array
            ...
            if self.img_array[image_row][image_col][0] > 220:  # if the pixel is white
                ...
                for _d in directions:
                    if (
                        0 <= print_row + _d[1] < square_width
                        and 0 <= print_col + _d[0] < square_width
                    ):  # if in the bounds of the surface_print array size
                        if (
                            surface_print[print_row + _d[1]][print_col + _d[0]] == 0
                        ):  # if the pixel has not an explorer already

                            surface_print[print_row + _d[1]][
                                print_col + _d[0]
                            ] = 1  # we indicate that we have added the coords to the explorers

                            new_explorer_x = print_col - self.group_size + _d[0]
                            new_explorer_y = print_row - self.group_size + _d[1]
                            explorers += [(new_explorer_x, new_explorer_y)]
                            nb_explorers += 1
            ...
            # nb_op+=1
        self.white_contigous_surface = self.nb_contiguous_white_pixel / (
...
class Row_Agent(object):
    ...
    def Fuse_RALs(self, _start, _stop):
        ...
        fusion_RAL = ReactiveAgent_Leader(
            _x=int(fusion_RAL_x / (_stop + 1 - _start)),
        ...
        if (
            self.RALs[_start].used_as_filling_bound
            and self.RALs[_stop].used_as_filling_bound
        ):
    ...
    def Fill_RALs(self, _RAL_1_index, _RAL_2_index, _filling_step):
        ...
        if (
            not self.RALs[_RAL_1_index].used_as_filling_bound
            or not self.RALs[_RAL_2_index].used_as_filling_bound
        ):
            # =============================================================================
            ...
            if nb_new_RALs > 0:
                ...
                self.RALs = (
                    self.RALs[: _RAL_1_index + 1] + new_RALs + self.RALs[_RAL_2_index:]
                )

                self.InterPlant_Diffs = (
                    self.InterPlant_Diffs[:_RAL_1_index]
                    + new_diffs
    ...
    def Fill_or_Fuse_RALs(self, _crit_value, _fuse_factor=0.5, _fill_factor=1.5):
        ...
        while i < nb_RALs - 1:
            ...
            if self.InterPlant_Diffs[i] < _fuse_factor * _crit_value or (
                abs(self.RALs[i].x - self.RALs[i + 1].x) < min_size
                and abs(self.RALs[i].y - self.RALs[i + 1].y) < min_size
            ):
            ...
            if not self.extensive_init:
                if i < len(
                    self.InterPlant_Diffs
                ):  # in case we fused the last 2 RAL of the crop row
    ...
    def Move_RALs_to_active_points(self):
        for _RAL in self.RALs:
            _RAL.Move_Based_on_AD_Order(
                _RAL.active_RA_Point[0], _RAL.active_RA_Point[1]
            )
    ...
    def Adapt_RALs_group_size(self):
        for _RAL in self.RALs:
            if (
                _RAL.recorded_Decision_Score[-1] < 0.2
                and _RAL.group_size > 5 * _RAL.group_step
            ):
                _RAL.group_size -= 1
                _RAL.RAs_square_init()
            elif (
                _RAL.recorded_Decision_Score[-1] > 0.8
                and _RAL.group_size < 50 * _RAL.group_step
            ):
                _RAL.group_size += 1
                _RAL.RAs_square_init()
...
class Agents_Director(object):
    ...
    def Analyse_RowAs(self):
        ...
        for i in range(nb_to_delete):
            self.RowAs = (
                self.RowAs[: to_delete[i] - i] + self.RowAs[to_delete[i] - i + 1 :]
            )
    ...
    def Analyse_RowAs_Kmeans(self):
        ...
        if p_value < 0.0001:
            ...
            index_small_grp = list(
                np.array((_indeces_grp0, _indeces_grp1))[
                    np.where(means_grp == min(means_grp))
                ][0][0]
            )
            ...
            while k < nb_indeces:
                ...
                while (
                    i < nb_indeces - 1
                    and index_small_grp[i] == index_small_grp[i + 1] - 1
                ):
    ...
    def ORDER_RowAs_Fill_or_Fuse_RALs(self):
        for _RowA in self.RowAs:
            _RowA.Fill_or_Fuse_RALs(
                self.InterPlant_Y, self.RALs_fuse_factor, self.RALs_fill_factor
            )
    ...
    def Check_RowAs_Proximity(self):
        ...
        while i < nb_Rows - 1:
            if (
                abs(self.RowAs[i].Row_Mean_X - self.RowAs[i + 1].Row_Mean_X)
                < self.group_size
            ):

                new_plant_FT = (
                    self.RowAs[i].plant_FT_pred_in_crop_row
                    + self.RowAs[i].plant_FT_pred_in_crop_row
                )
                new_plant_FT.sort()
...
class Simulation_MAS(object):
    ...
    def Initialize_AD(self):
        self.AD = Agents_Director(
            self.plant_FT_pred_par_crop_rows,
            self.OTSU_img_array,
            self.group_size,
            self.group_step,
    ...
    def Perform_Simulation_newEndCrit(
        self,
        _steps=10,
        _coerced_X=False,
        _coerced_Y=False,
    ...
    def Perform_Simulation_Extensive_Init(
        self,
        _steps=10,
        _coerced_X=False,
        _coerced_Y=False,
    ...
    def Correct_Adjusted_plant_positions(self):
        ...
        for adj_pos_string in self.ADJUSTED_img_plant_positions:
            ...
            self.corrected_adjusted_plant_positions += [
                [int(x), self.OTSU_img_array.shape[0] - int(y)]
            ]
    ...
    def Is_Plant_in_RAL_scanning_zone(self, _plant_pos, _RAL):
        ...
        res = False
        if (
            abs(_plant_pos[0] - _RAL.x) <= _RAL.group_size
            and abs(_plant_pos[1] - _RAL.y) <= _RAL.group_size
        ):
            res = True
        return res
    ...
    def Compute_Scores(self):
        ...
        for i in range(self.nb_real_plants):
            ...
            for _RowA in self.AD.RowAs:
                for _RAL in _RowA.RALs:
                    if self.Is_Plant_in_RAL_scanning_zone(
                        self.corrected_adjusted_plant_positions[i], _RAL
                    ):
                        if not TP_found:
                            self.TP += 1
                            TP_found = True
            ...
            for _RAL in _RowsA.RALs:
                ...
                for k in range(nb_indeces):
                    rect = patches.Rectangle(
                        (
                            _RAL.recorded_positions[_recorded_position_indeces[k]][0]
                            - _RAL.group_size,
                            _RAL.recorded_positions[_recorded_position_indeces[k]][1]
                            - _RAL.group_size,
                        ),
                        2 * _RAL.group_size,
                        2 * _RAL.group_size,
    ...
    def Show_Adjusted_Positions(self, _ax=None, _color="b"):
        ...
        for [x, y] in self.corrected_adjusted_plant_positions:
            circle = patches.Circle(
                (x, y), radius=2, linewidth=2, edgecolor=None, facecolor=_color
            )
            ax.add_patch(circle)

    def Show_Adjusted_And_RALs_positions(
        self,
        ...
        self.Show_Adjusted_Positions(_ax=ax, _color=_color_adjusted)
    ...
    def Show_RALs_Deicision_Scores(self):
        ...
        for _RowsA in self.AD.RowAs:
            for _RAL in _RowsA.RALs:
                ax.plot(
                    [i for i in range(len(_RAL.recorded_Decision_Score))],
                    _RAL.recorded_Decision_Score,
                    marker="o",
                )
    ...
    def Show_nb_RALs(self):
        ...
        ax = fig.add_subplot(111)
        ax.plot(
            [i for i in range(len(self.RALs_recorded_count))],
            self.RALs_recorded_count,
            marker="o",
        )
...
class MetaSimulation(object):
    ...
    def check_data(self):
        ...
        for _data in [self.data_input_OTSU, self.data_input_PLANT_FT_PRED]:
    ...
    def Get_Field_Assembling_Offsets(self):
        ...
        origin_shape = np.array(
            [self.data_input_raw[0].shape[1], self.data_input_raw[0].shape[0]]
        )
        Otsu_shape = np.array(
            [self.data_input_OTSU[0].shape[1], self.data_input_OTSU[0].shape[0]]
        )

        p1 = np.array(
            [self.data_input_raw[0].shape[1], 0.5 * self.data_input_raw[0].shape[0]]
        )
        p2 = np.array(
            [0.5 * self.data_input_raw[0].shape[1], self.data_input_raw[0].shape[0]]
        )
        pivot = np.array(
            [
                0.5 * self.data_input_raw[0].shape[1],
        ...
        for i in range(self.field_shape[0]):
            ...
            for j in range(_start, _stop, _step):
                ...
                self.all_offsets.append(
                    [int(new_offset[0]), int(Otsu_shape[1] - new_offset[1])]
                )
    ...
    def Launch_Meta_Simu_Labels(
        ...
        for i in range(self.nb_images):
            ...
            print()
            print(
                "Simulation Definition for image {0}/{1}".format(i + 1, self.nb_images)
            )
            ...
            try:
                ...
                if self.extensive_Init:
                    MAS_Simulation.Perform_Simulation_Extensive_Init(
                    ...
                    MAS_Simulation.Perform_Simulation_newEndCrit(
                        self.simulation_step,
                        self.coerced_X,
                        self.coerced_Y,
                        self.analyse_and_remove_Rows,
                    ...
                    MAS_Simulation.Perform_Simulation(
                        self.simulation_step,
                        self.coerced_X,
                        self.coerced_Y,
                        self.analyse_and_remove_Rows,
                ...
                if MAS_Simulation.max_steps_reached:
                    self.log += [
                        "Simulation for image {0}/{1}, named {2} reached max number of allowed steps".format(
                ...
                print("Failure")
                self.log += [
                    "Simulation for image {0}/{1}, named {2} failed".format(
                        i + 1, self.nb_images, self.names_input_raw[i]
                    )
                ]
                raise
    ...
    def Get_Simulation_Results(self, _MAS_Simulation):
        ...
        data = {
            "Time_per_steps": _MAS_Simulation.simu_steps_times,
    ...
    def Add_Simulation_Results(self, _image_index, _MAS_Simulation):
        ...
        self.RALs_data[
            self.names_input_raw[_image_index]
        ] = _MAS_Simulation.RALs_dict_infos
