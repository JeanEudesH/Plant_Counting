...
def writer(_path, _file_name, _content, _overwrite=False, _print_file_status=True):
    ...
    if not os.path.exists(_path + "/" + _file_name):
        ...
        if _overwrite:
            ...
            if _print_file_status:
                print(
                    "File {0} already exists in directory {1} and overwrite is set to False".format(
                        _file_name, _path
                    )
                )
...
def copier(_path, _file_name, _new_path, _new_file_name, _overwrite=False):
    ...
    if not os.path.exists(_new_path + "/" + _new_file_name):
        ...
        print(
            "File {0} was successfully copied in {1}".format(_new_file_name, _new_path)
        )
        ...
        if _overwrite:
            ...
            print(
                "File {0} was successfully copied (and overwritten) in {1}".format(
                    _new_file_name, _new_path
                )
            )
...
def concatener(
    _path,
    _file_name,
    _path_files_to_concatenate,
    _files_to_concatenate,
    _overwrite=False,
):
