...
def separate_X_Y_from_bsas_files(_data):
    ...
    for _coord in _data[1:]:
        (y, x) = _coord.split(
            ","
        )  # y is first because coordinates in bsas files are (line, column)
        X += [float(x)]
        Y += [float(y)]
...
def Get_Corrected_Peak_Index(_histogram, _peak_index, _search_window_half_width):
    subset_low_boundary = Clamp_Value(
        _peak_index - _search_window_half_width, 0, _histogram.size - 1
    )
    subset_high_boundary = Clamp_Value(
        _peak_index + _search_window_half_width + 1, 0, _histogram.size - 1
    )
...
def Search_Periodic_Peaks(_histogram, _period, _bin_div):
    ...
    while peak_index > 0:
        correction_to_global_max_index = Get_Corrected_Peak_Index(
            _histogram, peak_index, search_window_half_width
        )
...
def Extract_Y_Coord_of_Crop_Rows(
    _crop_rows, _x_data_size, _x_period, _x_coord, _y_coord
):
    ...
    i = 0
    subset_low_boundary = Clamp_Value(
        _crop_rows[crops_rows_count] - window_half_width, 0, _x_data_size - 1
    )
    subset_high_boundary = Clamp_Value(
        _crop_rows[crops_rows_count] + window_half_width + 1, 0, _x_data_size - 1
    )
    _row_content = []
        ...
        if _x_coord_sorted[i] > subset_low_boundary:
            if _x_coord_sorted[i] < subset_high_boundary:
                ...
                if crops_rows_count < nb_crops_rows:
                    subset_low_boundary = Clamp_Value(
                        _crop_rows[crops_rows_count] - window_half_width,
                        0,
                        _x_data_size - 1,
                    )
                    subset_high_boundary = Clamp_Value(
                        _crop_rows[crops_rows_count] + window_half_width + 1,
                        0,
                        _x_data_size - 1,
                    )
                else:
...
def Get_Signal_Period(_data, _axis_size, _bin_div):
    histogram = np.histogram(
        _data, bins=int(_axis_size / _bin_div), range=(0, _axis_size)
    )
...
def All_Fourier_Analysis(
    _path_input_output, _session_number=1, _bin_div_X=2, _bin_div_Y=4
):
    ...
    ################## Import Data
    data_bsas_dir0 = import_data(
        path_input_bsas_dir0, names_input_bsas_dir0[:subset_size], get_file_lines
    )

    data_bsas_dir1 = import_data(
        path_input_bsas_dir1, names_input_bsas_dir1[:subset_size], get_file_lines
    )
    ...
    for i in range(nb_images):
        ...
        crops_rows_content = Extract_Y_Coord_of_Crop_Rows(
            crops_rows, X2.size, signal_period * _bin_div_X, X2, Y2
        )
        ...
        for j in range(nb_rows):
            predicted_plants = Search_Periodic_Peaks(
                all_histograms_per_CR[j][0], signal_period, _bin_div_Y
            )
...
if __name__ == "__main__":
    ...
    All_Fourier_Analysis(
        _path_input_output="/home/fort/Documents/APT 3A/Cours/Ekinocs/Ouput_General",
        _session_number=1,
        _bin_div_X=2,
        _bin_div_Y=4,
    )
